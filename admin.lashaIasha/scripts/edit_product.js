...
document.getElementById("submit").addEventListener("click",(e)=>{
  ...
  if(title.value.length == 0){
    ...
    e.preventDefault();
      ...
      e.preventDefault();
  }else if(images.files.length == 0 && size_of_images === 0){
...
product_link.addEventListener("change", (e)=>{
  if(product_link.value.length == 0){
    ...
    check_link = false;
  }else if (current_product_link === product_link.value) {
    check_link = true;
  }else{
...
document.addEventListener("click",(e)=>{
  if(e.target.id == "image_delete" || e.target.id == "video_delete"){
    const data = new FormData();
    if(e.target.id == "image_delete"){
      data.append("delete_image",e.target.parentElement.lastElementChild.value);
    }else{
      data.append("delete_video",e.target.parentElement.lastElementChild.value);
    }
